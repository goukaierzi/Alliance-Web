<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html >
<html>
<head>
<meta charset=“UTF-8">
<title>Insert title here</title>
<meta charset="utf-8">
			<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width,height=device-height,initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no">
		<link rel="stylesheet" type="text/css" href="../plugins/bootstrap-3.3.7-dist/css/sbootstrap.min.css"/>
		<link href="../css/smod1.css" rel="stylesheet" type="text/css">
		<link href="../css/smod1_1.css" rel="stylesheet" type="text/css">
		<link href="../css/smod2.css" rel="stylesheet" type="text/css">
		<link href="../css/smod2_1.css" rel="stylesheet" type="text/css">
		<link href="../css/smod3.css" rel="stylesheet" type="text/css">
		<link href="../css/smod4.css" rel="stylesheet" type="text/css">
		<link href="../css/smod5.css" rel="stylesheet" type="text/css">
		<link href="../css/smod7.css" rel="stylesheet" type="text/css">
		<link href="../css/smod8.css" rel="stylesheet" type="text/css">
	    <style type="text/css">
			.home{
				background: url(../img/0d280d35970a304ed5a3178bd8c8a786c8175cf3.jpg);
					}
			#gouying{
				display: block;/*将a变成块状*/
				width: 600px;/*设置块的宽度*/
				height: 100px;/*设置块的长度*/
				font-family: Microsoft Yahei;
				line-height: 50px;/*设置字体在块中的高度*/
				margin: 0px 0px;/*块里的高宽通过margin设置*/
				color: #fff;
				text-align: left;/*字体居中*/
				text-decoration: none;/*去掉下划线*/
				margin-top: 80px;
				margin-left: 80px;
				font-size: 35px;
				position: absolute;
			}
			#swt{
				display: block;/*将a变成块状*/
				border: 3px solid #EEA236;
				width: 600px;/*设置块的宽度*/
				height: 200px;/*设置块的长度*/
				line-height: 50px;/*设置字体在块中的高度*/
				margin: 2px 2px;/*块里的高宽通过margin设置*/
				text-align: left;/*字体居中*/
				color: #00C3FF;
				text-decoration: none;/*去掉下划线*/
				margin-top: 180px;
				margin-left: 100px;
				font-size: 20px;
				position: absolute;
				
			}
			.c1{
				font-size: 15px;
				color: #3C763D;
			}
			.c2{
				left: 650px;
			}
			.c3{
				top: 250px;
			}
			.c4{
				top: 250px;
				left: 650px;
			}
			.c5{
				top: 510px;
			}

				</style>
</head>
    <body class="home">
		<div class="navbars navbars_notTop">
			<nav class="nav_notTop" role="main">
				<div class="nav_left">
					<a  href="#" ></a>
				</div>
				<ul class="nav_menu nav_left">
					<li>
						<a class="nav_btn" href="首页.jsp">首页</a>
					</li>
					<li>
						<a class="nav_btn" href="英雄资料.jsp">资料</a>
					</li>
					<li>
						<a class="nav_btn" href="活动.jsp">活动</a>
					</li>
					<li>
						<a class="nav_btn" href="精彩集锦.jsp">精彩集锦</a>
					</li>
					<li>
						<a class="nav_btn" href="新闻.jsp">新闻</a>
					</li>
					<li>
						<a class="nav_btn" href="下载页面.jsp">下载</a>
					</li>
					<li class="dropdown">
                		<a href="#" class="nav_btn" data-toggle="dropdown" >
                			商城
                			<b class="caret"></b>
                		</a>
                		<ul class="dropdown-menu">
                			<li><a href="英雄1.jsp">英雄</a></li>
                    		<li><a href="皮肤.jsp">皮肤</a></li><hr />
                    		<li><a href="道具.jsp">道具</a></li>
                    	</ul>
            		</li>
            		     	
            	<li>
                	<a href="#" class="nav_btn">论坛</a>
            	</li>
            	<li>
                	<a href="#" class="nav_btn" style="margin-left: 500px;">登录</a>
            	</li>
            	<li>
                	<a href="#" class="nav_btn" >注册</a>
            	</li>
        </ul>       
			</nav>
		</div>		
		<div id="gouying">
			<b>《王者联盟》官方论坛</b><br>
			<b class="c1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;综合讨论区</b>

		</div>
		<div id="swt">
			<a><font color="#00A5E2">信息发布专区<br>  本版块为《守望先锋》信息发布专区，您可以在这里浏览来自官方的最新信息。</font></a>
   		</div>
   		<div id="swt" class="c2" >
			<a ><font color="#00A5E2">综合讨论区<br>  我曾以为战斗才是一切，直到我来到了这里……</font></a>
   		</div>
   		<div id="swt" class="c3" >
			<a ><font color="#00A5E2">汉化及配音讨论区<br>     您可以在这里对《守望先锋》国服的语音、文字等任何游戏内容的本地化改变进行讨论，暴雪中国本地化团队会在这里聆听你的声音！</font></a>
   		</div>
		<div id="swt" class="c4" >
			<a ><font color="#00A5E2">战友招募区 <br> ——坦克？辅助？谢谢 <br>——喂，推车啊不要乱跑！</font></a>
   		</div>
   		<div id="swt" class="c5" >
			<a ><font color="#00A5E2">战友招募区 <br> ——坦克？辅助？谢谢 <br>——喂，推车啊不要乱跑！</font></a>
   		</div>
		<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
		<footer id="footer">
			<div class="footerWrap">
				<div class="support-feedback clearFix">
					<div class="cs">
						<a class="cs-online" href="" target="_blank">
							<span class="icon icon-1"></span> 在线客服
						</a>
						<a class="cs-Feedback" href="" target="_blank">
							<span class="icon icon-2"></span> 反馈
						</a>
						<a class="cs-joinus" href="" target="_blank">
							<span class="icon icon-3"></span> 加入我们
						</a>
					</div>
				</div>
				<div class="cprt">
					<a class="logo_bz" href="" target="_blank"></a>
					<a class="logo_ne" href="" target="_blank"></a>
					<div class="info">
						<ul class="f-cb">
							<li>
								<a href="" target="_blank">隐私</a>|</li>
							<li>
								<a href="" target="_blank">法律条款</a>|</li>
							<li>
								<a href="" target="_blank">API</a>
							</li>
						</ul>
						<p>©2018 kkwbringfly有限公司版权所有 由pkk最帅网络科技发展有限公司运营
							<a href=""> 著作权侵权</a>&nbsp;新广出审[2016]378号</p>
						<p>文网游进字[2016]0025号&nbsp;
							<a href="">沪网文[2014]0731-161号</a>&nbsp;
							<a href="">增值电信业务经营许可证编号：江B2－20080012</a>
						</p>
						<p>互联网违法和不良信息举报电话：110!！&nbsp;沪公网安备 31011502032167号</p>
						<p class="integrity">
							<a href="" target="_blank"><img src="img/footer_logo/police.png"></a>
							<a href="" target="_blank"><img src="img/footer_logo/zx110.png"></a>
							<a href="" target="_blank"><img src="img/footer_logo/sgs-icon.png"></a>
							<a href="" target="_blank"><img src="img/footer_logo/icon20120516.png"></a>
							<span>| &nbsp;适龄提示：适合33岁及以上使用&nbsp;&nbsp;</span>
							<a href="" target="_blank">家长监护工程&gt;</a>
						</p>
					</div>
				</div>
				<p class="tips">积极健康的游戏心态是健康游戏的开端，本游戏故事情节设置紧凑，请您合理控制游戏时间，避免沉溺游戏影响生活，注意自我保护，防范网络陷阱。
					<br>健康游戏忠告：抵制不良游戏，拒绝盗版游戏。注意自我保护，谨防受骗上当。适度游戏益脑，沉迷游戏伤身。合理安排时间，享受健康生活。</p>
			</div>
		</footer>

	</body>
</html>